...
import { Button, UserInput, ConfirmModal } from "components";
  ...
  const getOriginProductInfo = () => {
    ...
    try {
      ...
      const foundProduct = myProducts.find(
        (item: ItemInfoProps) => item._id === productId,
      );
  ...
  const putProduct = async () => {
    if (
      ...
      try {
        ...
        const productIndex = myProducts.findIndex(
          (item: ItemInfoProps) => item._id === productId,
        );
        ...
        if (productIndex !== -1) {
          myProducts[productIndex] = {
            ...myProducts[productIndex],
            name: inputValue.productName,
            image: hasFile ? preview : myProducts[productIndex].image,
            price: Number(inputValue.productPrice),
            description: inputValue.productDescription,
            categoryName: selectedCategory,
            updatedAt: new Date().toISOString(),
