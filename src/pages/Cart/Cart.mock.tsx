...
import { fetchMockCartData } from "../../utils/fetchMockCartData";
...
const CartPage: React.FC = () => {
  ...
  useEffect(() => {
    const loadCartData = async () => {
      const data = await fetchMockCartData();
      setCartData(data);
    };
  ...
  const calculateShopTotalAmount = (items: CartItemType[]) => {
    return items
      .filter((item) => item.checked)
      .reduce((sum, item) => sum + item.price, 0);
  };

  const calculateShopSelectedItemsCount = (items: CartItemType[]) => {
